...
public class GroupParser extends SelectParser {
	...
	public TermsBuilder parse(List<GroupingElement> elements, QueryState state){
		...
		for(Column groupby : groups){
			if(groupby.getOp() != Operation.NONE){
				state.addException("Can not use function  '"+groupby.getFullName()+"' as GROUP BY, please use an alias to group by a function");
				return null;
			}
		}
		...
		for(Column g : groups){
			Column s = state.getHeading().getColumnByLabel(g.getFullName());
			if(s == null ){
	...
	private TermsBuilder buildAggregationQuery(List<Column> aggs, int index, QueryState state){
		...
		if(aggs.get(index).getOp() == Operation.NONE){
			result = AggregationBuilders.terms(agg.getFullName()).field(agg.getColumn());
			//if(!req.hasOrder()) result.size(req.getLimit());
			//else 
	...
	private void addMetrics(AggregationBuilder parentAgg, Heading heading, boolean addCount){
		for(Column metric : heading.columns()){
			if(metric.getOp() == Operation.AVG) 
				parentAgg.subAggregation(AggregationBuilders.avg(metric.getFullName()).field(metric.getColumn()));
			else if(addCount && metric.getOp() == Operation.COUNT)
				parentAgg.subAggregation(AggregationBuilders.count(metric.getFullName()));
			else if(metric.getOp() == Operation.MAX) 
				parentAgg.subAggregation(AggregationBuilders.max(metric.getFullName()).field(metric.getColumn()));
			else if(metric.getOp() == Operation.MIN) 
				parentAgg.subAggregation(AggregationBuilders.min(metric.getFullName()).field(metric.getColumn()));
			else if(metric.getOp() == Operation.SUM) 
				parentAgg.subAggregation(AggregationBuilders.sum(metric.getFullName()).field(metric.getColumn()));
		}
	}
