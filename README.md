...
**Shield for SQL4ES 0.8.5.X** the following URL prameters must be set in order to use shield:

- xpack.security.user='username:password' to set global credentials
- xpack.security.transport.ssl.enabled=true to enable SSL

See this [Elastic page](https://www.elastic.co/guide/en/x-pack/current/java-clients.html) for other settings that can be used to configure the 'shielded connection'.
...
**Shield for SQL4ES 0.8.2.X** the following URL prameters must be set in order to use shield:
